...
func (r *queryResolver) Articles(ctx context.Context, limit *int32, offset *int32) ([]*model.Article, error) {
	...
	articles := []*model.Article{
		{
			...
			ImageURL:          stringPtr("https://picsum.photos/800/400"),
			SatiricalImageURL: stringPtr("https://picsum.photos/800/400"),
			SatiricalSummary:  stringPtr("This is a humorous take on sample article 1."),
		...
		},
		{
			...
			ImageURL:          stringPtr("https://picsum.photos/800/400"),
			SatiricalImageURL: stringPtr("https://picsum.photos/800/400"),
			SatiricalSummary:  stringPtr("This is a humorous take on sample article 2."),
	...
	// Process articles with AI if not already processed
	for _, article := range articles {
		if !article.IsProcessed {
			summary, imageURL, err := r.aiService.ProcessArticle(ctx, article.Content)
			if err != nil {
				log.Printf("Error processing article %s: %v", article.ID, err)
				continue
			}
			article.SatiricalSummary = &summary
			article.SatiricalImageURL = &imageURL
